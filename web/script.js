...
function renderMappings() {
  ...
  for (var i = 0; i < letters.length; i++) {
    ...
    if (LETTER_TO_COORD[letter]) {
      html += '<div class="map-item">' + letter + ' &rarr; ' + LETTER_TO_COORD[letter] + '</div>';
    } else {
      html += '<div class="map-item">Z &rarr; no coordinate</div>';
      break;
    }
  }
...
function encrypt(text) {
  ...
  for (var i = 0; i < upper.length; i++) {
    ...
    if (ch === 'Z') {
      throw new Error('Z is not present in the 5x5 grid shown in the workshop.');
    }
...
function decrypt(text) {
  ...
  for (var i = 0; i < tokens.length; i++) {
    ...
    if (!/^\d{2}$/.test(token) || !COORD_TO_LETTER[token]) {
      throw new Error('Invalid coordinate: ' + token + '. Use values such as 23 or 34.');
    }
